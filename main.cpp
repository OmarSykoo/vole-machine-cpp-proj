...
public:
    ...
    void run() {
        while (step != -1 && (step / 2 < data_.size())) {
            ...
            step = instructions[(((temp.first & 0xF0)) >> 4)]->process(
                    registers, addresses, (temp.first & 0x0F),temp.second, step);
        }
    }
    ...
    void fetch(const string &text) {
        ...
        int o , R, XY;
        while (fin >> hex >> o >> R >> XY) {
            data_.emplace_back(( ( o << 4 ) + R) , XY ) ;
        }
    }
    ...
    void print() {
        cout << "#N #R 00 01 02 03 04 05 06 07 08 09 0a 0b 0c 0d 0e 0f\n";
        for (int i = 0; i < 16; i++) {
            cout << hex << 0 << i << ' ';
            if (registers[i].get_value() < 16) {
                cout << 0;
            }
            cout << hex << registers[i].get_value() << ' ';
            for (int j = 0; j < 16; j++) {
                if (addresses[(j << 4) + i].get_value() < 16) {
                    cout << 0;
                }
